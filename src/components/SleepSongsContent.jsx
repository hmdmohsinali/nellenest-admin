...
const SleepSongsContent = () => {
  ...
  const fetchItems = async () => {
    try {
      ...
      setError(err.message || 'Failed to load sleep songs');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!confirm('Delete this sleep song?')) return;
    try {
      await adminAPI.sleepSongs.delete(id);
      fetchItems();
    } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingItem && (editingItem._id || editingItem.id)) {
        ...
        await adminAPI.sleepSongs.update(id, formData);
      } else {
        await adminAPI.sleepSongs.create(formData);
      }
      closeEditModal();
      await fetchItems();
    } catch (e) {
  ...
  return (
    <div className="py-6 lg:py-8 space-y-6 lg:space-y-8">
      <div className="mb-6 lg:mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-800 mb-2">Sleep Songs</h1>
        <p className="text-sm sm:text-base text-slate-600">Create, update, and manage sleep songs</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3">
        <div className="relative w-full sm:w-72">
          ...
          <input type="text" placeholder="Search sleep songs..." value={query} onChange={(e) => { setQuery(e.target.value); setPage(1); }} className="w-full pl-10 pr-4 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="bg-white divide-y divide-slate-200">
                {items.length === 0 ? (
                  <tr><td colSpan="5" className="px-6 py-8 text-center text-slate-500">No sleep songs found</td></tr>
                ) : (
                  items.map((item) => (
        ...
        <div className="fixed inset-0 backdrop-blur-sm bg-white/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white/95 backdrop-blur-md rounded-xl shadow-2xl border border-white/20 max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-slate-200">
              <h2 className="text-xl font-semibold text-slate-800">{editingItem ? 'Edit Sleep Song' : 'New Sleep Song'}</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <FileUpload
                    ...
                    folder="nellenest/sleep-songs/thumbnails"
                  />
                </div>
                <div>
                  ...
                  <FileUpload
                    ...
                    folder="nellenest/sleep-songs/files"
                  />
...
export default SleepSongsContent;
