  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-600">Track your key performance metrics and user behavior</p>
      </div>
            ...
            <div key={index} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
              <div className="flex items-center justify-between mb-4">
                <div className={`p-3 rounded-lg ${getColorClasses(metric.color)} bg-opacity-10`}>
                  <Icon className={`w-6 h-6 ${getColorClasses(metric.color)}`} />
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="space-y-3">
            {chartData.map((data, index) => (
              <div key={index} className="flex items-center space-x-3">
                <span className="text-sm text-slate-500 w-12">{data.date}</span>
        ...
        {/* Users Chart */}
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          <h3 className="text-lg font-semibold text-slate-800 mb-4">Unique Users</h3>
          <div className="space-y-3">
            {chartData.map((data, index) => (
              <div key={index} className="flex items-center space-x-3">
                <span className="text-sm text-slate-500 w-12">{data.date}</span>
          ...
          {[
            ...
            <div key={index} className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 transition-colors duration-200">
              <div className="flex-1">
                ...
                <p className="text-sm text-slate-500">Engagement: {content.engagement}</p>
              </div>
              <div className="text-right">
                <p className="font-medium text-slate-800">{content.views.toLocaleString()} views</p>
                <p className="text-sm text-green-600">{content.trend}</p>
              </div>
            </div>
          ))}
        </div>
